...
1. **基础**：基础设置（启用、立刻全量同步、任务完成通知、上传并发数、SSE 监听）
2. **播放 / 302**：播放入口设置（MoviePilot 内网地址、STRM URL 模式、解析最终直链、直链来源策略、直链缓存时间、HEAD 探测策略）+ Emby 302 前置代理（启用开关、Emby 原始地址、代理监听地址、代理监听端口）
3. **云存储 / 路径**：云端存储设置（云存储类型、AList/OpenList 地址、Token）+ 上传与 STRM 路径映射（上传映射、STRM 映射）
4. **同步 / 刷新**：同步与过滤（同步模式、STRM 覆盖模式、媒体扩展名、排除规则、事件路径过滤）+ 媒体服务器刷新（刷新开关、媒体服务器选择、路径映射）
...
以及两个列表，每条记录右侧带「操作」按钮，直接对该条记录执行单条操作（无需进配置页）：

- 最近上传列表（文件名 | 状态 | 大小 | 时间 | 云端路径 | 操作）
- 最近生成 STRM 列表（文件名 | 状态 | 时间 | STRM 路径 | 云端路径 | 操作）

最近上传列表的单条操作（按记录字段条件显示，无本地路径的记录不显示需要本地源的动作）：

- `重新上传`：先删除该条云端文件，再用本地源文件上传，并重新生成对应 STRM（仅 `已上传`/`远端已存在` 且有本地路径时可用）。
- `删除云端`：只删除该条云端文件，不动本地文件（warning 配色）。
- `删云端和本地`：删除云端文件后，再删除上传映射本地根目录内的对应本地文件（error 配色，文案显式标注后果）。

最近生成 STRM 列表的单条操作：

- `重新生成 STRM`：按当前 STRM URL 模式重新写入该条 STRM 文件内容。

所有单条操作通过 `/manual_action` API 在后台线程执行，破坏性操作（删除）用醒目配色 + 两步操作（点按钮即触发，后端再校验路径范围）防误触；不再通过配置页保存触发手动动作。
...
| `/manual_action` | `POST` | 首页列表内单条操作，参数 `action`（`reupload`/`delete_remote`/`delete_remote_and_local`/`regenerate_strm`）、`local`、`remote`、`strm` |
